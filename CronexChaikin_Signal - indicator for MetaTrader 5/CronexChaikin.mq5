...
//+-----------------------------------+
input ENUM_MA_METHOD ChaikinMethod=MODE_SMA; // метод усреднения Chaikin
input uint ChaikinFastPeriod=3;              // быстрый период Chaikin
input uint ChaikinSlowPeriod=10;             // медленный период Chaikin
...
//---- Инициализация переменных начала отсчёта данных
   min_rates_1=int(ChaikinFastPeriod+ChaikinSlowPeriod);
...
//--- получение хендла индикатора iChaikin
   Ind_Handle=iChaikin(Symbol(),PERIOD_CURRENT,ChaikinFastPeriod,ChaikinSlowPeriod,ChaikinMethod,VolumeType);
   if(Ind_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора iChaikin");
      return(INIT_FAILED);
     }
...
//--- создание имени для отображения в отдельном подокне и во всплывающей подсказке
   IndicatorSetString(INDICATOR_SHORTNAME,"CronexChaikin");
...
//стартового номера limit для цикла пересчёта баров
   if(prev_calculated>rates_total || prev_calculated<=0)// проверка на первый старт расчёта индикатора
     {
      limit=maxbar1; // стартовый номер для расчёта всех баров
     }
