...
#property indicator_color1  clrAqua,clrGray,clrYellow
//---- линия индикатора - сплошная
#property indicator_style1 STYLE_SOLID
//---- толщина линии индикатора равна 2
#property indicator_width1 2
//---- отображение метки индикатора
#property indicator_label1  "DeMarker_Histogram"

//+-----------------------------------+
//|  ВХОДНЫЕ ПАРАМЕТРЫ ИНДИКАТОРА     |
//+-----------------------------------+
input uint                 DeMarkerPeriod=14;     // период индикатора
input double               HighLevel=0.7;         // уровень перезакупа
input double               LowLevel=0.3;          // уровень перепроданности
...
//--- создание имени для отображения в отдельном подокне и во всплывающей подсказке
   IndicatorSetString(INDICATOR_SHORTNAME,"DeMarker_Histogram("+string(DeMarkerPeriod)+")");
...
//---- проверка количества баров на достаточность для расчёта
   if(BarsCalculated(DeMarker_Handle)<rates_total || rates_total<min_rates_total) return(RESET);
...
//---- копируем вновь появившиеся данные в массивы
   if(CopyBuffer(DeMarker_Handle,0,0,to_copy,UpBuffer)<=0) return(RESET);
