...
#property description "Индикатор DeMarker на ценовом графике"
...
//+----------------------------------------------+
//|Параметры отрисовки индикатора DeMarker Cloud |
//+----------------------------------------------+
...
#property indicator_label1  "DeMarker Cloud"
//+----------------------------------------------+
...
//---- отображение метки индикатора
#property indicator_label3  "DeMarker"
//+----------------------------------------------+
//|  Объявление констант                         |
//+----------------------------------------------+
...
double Line4Buffer[];
//----
double dLevel_DeMarker_UP,dLevel_DeMarker_DN;
...
//----
   dLevel_DeMarker_UP=int(Level_DeMarker_UP-50)*_Point*Dev;
   dLevel_DeMarker_DN=int(Level_DeMarker_DN-50)*_Point*Dev;
//--- получение хендла индикатора DeMarker
   Ind_Handle=iDeMarker(Symbol(),NULL,PeriodDeMarker);
...
//---- инициализация переменной для короткого имени индикатора
   string shortname;
   StringConcatenate(shortname,"F_DeMarker(",PeriodDeMarker,")");
...
//---- копируем вновь появившиеся данные в массивы
   if(CopyBuffer(Ind_Handle,MAIN_LINE,0,to_copy,DeMarker)<=0) return(RESET);
   ...
   for(bar=limit; bar>=0 && !IsStopped(); bar--)
     {
      ...
      Line3Buffer[bar]=xma;
      Line4Buffer[bar]=xma+Dev*(100*DeMarker[bar]-50)*_Point;
      Line1Buffer[bar]=xma+dLevel_DeMarker_UP;
      Line2Buffer[bar]=xma+dLevel_DeMarker_DN;
     }
//----     
   return(rates_total);
  }
